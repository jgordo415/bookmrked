...
const BASE_URL = "https://bookmrked.com";
  ...
  server: {
    handlers: {
      GET: async () => {
        ...
        // Shared collections (/c/$shareToken) are intentionally omitted: there is
        // no public listing of share tokens, only per-token access.
        if (entries.length === 0) {
          return new Response(
